...
for kk = 1:length(SNRdB)
    ...
    for monte=1:iter
        ...
        sent_syms = double(total_symbols(monte,:))';
        qam_syms = qammod(sent_syms, M)/sqrt(10);
        sn = reshape(qam_syms,Mr,1);
        
        
        %% Channel
        H_original = (1/sqrt(2))*(randn(Mr,Mt)+1i*randn(Mr,Mt));  % Main channel for the legitimate receiver, Bob.
        H_real = squeeze(total_main_channels_real(monte,:,:));
        H_imag = squeeze(total_main_channels_imag(monte,:,:));
        H = H_real + 1i*H_imag;
        ...
        x_zf_gd_papr_orignal = single(sqrt(theta/nf)*d + sqrt((1-theta)/nV)*V*AN);
                
        x_zf_gd_papr_real = total_nn_out_real(monte,:).';
        x_zf_gd_papr_imag = total_nn_out_imag(monte,:).';
        x_zf_gd_papr = x_zf_gd_papr_real + 1i*x_zf_gd_papr_imag;
